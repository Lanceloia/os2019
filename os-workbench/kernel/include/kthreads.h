...
enum { NIL = 0, STARTED = 1, RUNNABLE = 2, RUNNING = 3, YIELD = 4, KILLED = 5 };
...
void pushcli() {
  cli();
  if (ncli[_cpu()] == 0) intena[_cpu()] = get_efl() & FL_IF;
...
int holding(spinlock_t *lk) { return lk->locked && lk->cpu == _cpu(); }
