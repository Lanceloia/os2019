...
static void insert_node(int ppid, int pid, char name[],
                        int (*cmp)(list_node*, list_node*, list_node*,
                                   list_node*)) {
  ...
  list_node* p2 = p->next;
  while (p2 != Tree[ppid].tail &&
         (*cmp)(new_node, p2, Tree[ppid].head, Tree[ppid].tail)) {
    p = p->next;
    p2 = p2->next;
  }
...
void dfs_print_tree(int root, char name[], int depth, int max_valid,
                    int show_pids) {
  len_mem[depth] =
      show_pids ? printf("-%s(%d)", name, root) : printf("-%s", name);

  if (Tree[root].head != NULL)
    for (list_node* x = Tree[root].head->next; x != Tree[root].tail;
         x = x->next) {
      if (x->next != Tree[root].tail) {
        ...
        for (int i = 0; i < depth; i++)
          if (dfs_mem[depth]) max_valid = i;
      }

      dfs_print_tree(x->child_pid, x->child_name, depth + 1, max_valid,
                     show_pids);
...
void read_proc_status_do(char filepath[], int (*cmp)(list_node*, list_node*,
                                                     list_node*, list_node*)) {
...
void read_proc_status(char _pid[], int (*cmp)(list_node*, list_node*,
                                              list_node*, list_node*)) {
...
int execute(int function_flag) {
  if (function_flag & VERSION) return version();
